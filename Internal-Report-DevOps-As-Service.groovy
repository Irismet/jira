...
public ArrayList<Issue> selectTotalIssues(){
    ...
    if (start_date != "" && end_date != ""){
        query = jqlQueryParser.parseQuery("project in (DASIN, PCLOUD) and issuetype in (\"Запрос проектной команды (DevOps as Service)\", \"Devops as Service\") and created >= ${start_date} and created <= ${end_date}")
    }
    else {
        query = jqlQueryParser.parseQuery("project in (DASIN, PCLOUD) and issuetype in (\"Запрос проектной команды (DevOps as Service)\", \"Devops as Service\") and created >= startOfMonth() and created <= endOfMonth()")
    }
...
public ArrayList<Issue> selectLineResolvedIssues(String lineNumber){
    ...
    if (lineNumber == "2" && (start_date != "" && end_date != "")){
        query = jqlQueryParser.parseQuery("project in (DASIN, PCLOUD) and issuetype in (\"Запрос проектной команды (DevOps as Service)\", \"Devops as Service\") and status changed FROM \"${lineNumber} линия\" to \"Решен\" and status was not in (\"3 линия\") and status = Решен and resolved >= ${start_date} and resolved <= ${end_date}")
    }
    else if (lineNumber == "2" && (start_date == "" && end_date == "")){
        query = jqlQueryParser.parseQuery("project in (DASIN, PCLOUD) and issuetype in (\"Запрос проектной команды (DevOps as Service)\", \"Devops as Service\") and status changed FROM (\"${lineNumber} линия\") to \"Решен\" and status was not in \"3 линия\" and status = Решен and resolved >= startOfMonth() and resolved <= endOfMonth()")
        }
    else if (lineNumber == "3" && (start_date != "" && end_date != "")){
        query = jqlQueryParser.parseQuery("project in (DASIN, PCLOUD) and issuetype in (\"Запрос проектной команды (DevOps as Service)\", \"Devops as Service\") and status changed FROM (\"${lineNumber} линия\") to \"Решен\" and status = Решен and resolved >= ${start_date} and resolved <= ${end_date}")
    }
    else if (lineNumber == "3" && (start_date == "" && end_date == "")){
        query = jqlQueryParser.parseQuery("project in (DASIN, PCLOUD) and issuetype in (\"Запрос проектной команды (DevOps as Service)\", \"Devops as Service\") and status changed FROM (\"${lineNumber} линия\") to \"Решен\" and status = Решен and resolved >= startOfMonth() and resolved <= endOfMonth()")
    }
    else {
...
public ArrayList<Issue> selectLineNonResolvedIssues(String lineNumber){
    ArrayList<Issue> issueList = new ArrayList()
    if (start_date != "" && end_date != ""){
        query = jqlQueryParser.parseQuery("project in (DASIN, PCLOUD) and issuetype in (\"Запрос проектной команды (DevOps as Service)\", \"Devops as Service\") and status = \"${lineNumber} линия\" and created >= ${start_date} and created <= ${end_date}")
    }
    else {
        query = jqlQueryParser.parseQuery("project in (DASIN, PCLOUD) and issuetype in (\"Запрос проектной команды (DevOps as Service)\", \"Devops as Service\") and status = \"${lineNumber} линия\" and created >= startOfMonth() and created <= endOfMonth()")
    }
    //log.info query.getQueryString()
    def results = searchProvider.search(user, query, PagerFilter.getUnlimitedFilter())
...
def getTimeResolutionByProjectTeam(String project_team, CustomField time_resolution){
    try {
        ...
        if (project_team != "empty" && start_date != "" && end_date != ""){
            query = jqlQueryParser.parseQuery("project in (DASIN, PCLOUD) and issuetype in (\"Запрос проектной команды (DevOps as Service)\", \"Devops as Service\") and status changed from \"3 линия\" to \"Решен\" and status = \"Решен\" and \"Проектная команда\" = \"${project_team}\" and resolved >= ${start_date} and resolved <= ${end_date}")
        }
        else if (project_team != "empty" && start_date == "" && end_date == ""){
            query = jqlQueryParser.parseQuery("project in (DASIN, PCLOUD) and issuetype in (\"Запрос проектной команды (DevOps as Service)\", \"Devops as Service\") and status changed from \"3 линия\" to \"Решен\" and status = \"Решен\" and \"Проектная команда\" = \"${project_team}\" and resolved >= startOfMonth() and resolved <= endOfMonth()")
        }
        else{
            query = jqlQueryParser.parseQuery("project in (DASIN, PCLOUD) and issuetype in (\"Запрос проектной команды (DevOps as Service)\", \"Devops as Service\") and status changed from \"3 линия\" to \"Решен\" and status = \"Решен\" and \"Проектная команда\" is empty and resolved >= startOfMonth() and resolved <= endOfMonth()")
        }
        //log.info query.getQueryString()
        def results = searchProvider.search(user, query, PagerFilter.getUnlimitedFilter())
...
def getCountResolvedIssuesByAssignee(String assignee, String fromStatusName, String toStatusName){
    if (start_date != "" && end_date != ""){
        query = jqlQueryParser.parseQuery("project in (DASIN, PCLOUD) and issuetype in (\"Запрос проектной команды (DevOps as Service)\", \"Devops as Service\") and status changed from \"${fromStatusName}\" to \"${toStatusName}\" and status was not in (\"3 линия\") and status = \"Решен\" and assignee = ${assignee} and resolved >= ${start_date} and resolved <= ${end_date}")
    }
    else {
        query = jqlQueryParser.parseQuery("project in (DASIN, PCLOUD) and issuetype in (\"Запрос проектной команды (DevOps as Service)\", \"Devops as Service\") and status changed from \"${fromStatusName}\" to \"${toStatusName}\" and status was not in (\"3 линия\") and status = \"Решен\" and assignee = ${assignee} and resolved >= startOfMonth() and resolved <= endOfMonth()")
    }
    //log.info query.getQueryString()
    def results = searchProvider.search(user, query, PagerFilter.getUnlimitedFilter())
...
if (start_date != "" && end_date != ""){
    body = "<h2>Отчет по услуге DevOps as Service для внутрениих клиентов за период с ${start_date} по ${end_date}.</h2></br>"
    summary = "Отчет по услуге DevOps as Service для внутрениих клиентов за период с ${start_date} по ${end_date}."
}
else { 
    body = "<h2>Отчет по услуге DevOps as Service для внутрениих клиентов за ${current_month_value} месяц.</h2></br>"
    summary = "Отчет по услуге DevOps as Service для внутрениих клиентов за ${current_month_value} месяц."
}
